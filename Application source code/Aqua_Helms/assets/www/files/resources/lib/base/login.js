...
(function ($, Apperyio, undefined) {
     ...
     * @returns {object}
     */
    function loginOauth2_0(dbId, clientId, baseUrl, callbackUrl, socialnetwork, scope, needState) {
        var state = "";
        if (needState) {
        ...
        var url = baseUrl + "?client_id=" + clientId +
            "&redirect_uri=" + callbackUrl + "&scope=" + scope + "&response_type=code" + state;
        var deferred = getDeferred();
        var ref;
    ...
    Apperyio.User = {
        ...
        loginFB: function (clientId, callbackUrl, dbId) {
            return loginOauth2_0(dbId, clientId, providers.facebook.baseUrl, callbackUrl, providers.facebook.id, "", true);
        },
